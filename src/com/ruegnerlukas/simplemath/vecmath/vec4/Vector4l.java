...
import com.ruegnerlukas.simplemath.vecmath.vec3.Vector3l;
...
public class Vector4l implements IVector4 {
	...
	public Vector4l set(int index, Number value) {
		...
		if(index == 3) { this.w = value.longValue(); return this; }
		return null;
	}
	...
	public boolean compare(IVector4 vec) {
		if(Math.abs(x - vec.getLongX()) > 0) { return false; }
		if(Math.abs(y - vec.getLongY()) > 0) { return false; }
		if(Math.abs(z - vec.getLongZ()) > 0) { return false; }
		if(Math.abs(w - vec.getLongW()) > 0) { return false; }
		return true;
	}
