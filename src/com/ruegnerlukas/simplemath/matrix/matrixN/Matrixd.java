...
public class Matrixd implements IMatrixd {
	 ...
	 * */
	public static Matrixd createIdentity(int size) {
		Matrixd mat = null;
		if(size == 4) {
			mat = new Matrix4d();
		} else {
			mat = new Matrixd(size, size);
		}
	 ...
	 * */
	public Matrixd(int columns, int rows, double fill) {
	 ...
	 * */
	public Matrixd(IVector[] vectors, boolean asColumns) {
		...
		for(int i=0; i<this.columns; i++) {
			for(int j=0; j<this.rows; j++) {
				this.data[i][j] = asColumns ? vectors[i].getDouble(j) : vectors[j].getDouble(i);
			}
		}
	...
	public IVector getRowVector(int row) {
		...
		if(n == 2) {
			return new Vector2d(data[0][row], data[1][row]);
		} else if(n ==3) {
			return new Vector3d(data[0][row], data[1][row], data[2][row]);
		} else if(n == 4) {
			return new Vector4d(data[0][row], data[1][row], data[2][row], data[3][row]);
		} else {
			return new VectorNd(getRow(row));
		}
	}
	...
	public IVector getColumnVector(int column) {
		...
		if(n == 2) {
			return new Vector2d(data[column][0], data[column][1]);
		} else if(n == 3) {
			return new Vector3d(data[column][0], data[column][1], data[column][2]);
		} else if(n == 4) {
			return new Vector4d(data[column][0], data[column][1], data[column][2], data[column][3]);
		} else {
			return new VectorNd(getColumn(column));
		}
	}
	...
	@Override
	public Matrixd getSubmatrix(int colStart, int rowStart, int colEnd, int rowEnd) {
		Matrixd mat = new Matrixd(colEnd-colStart+1, rowEnd-rowStart+1);
	...
	@Override
	public Matrixd switchColumns(int c0, int c1) {
		if(!unsafe) {
	...
	@Override
	public Matrixd switchRows(int r0, int r1) {
		if(!unsafe) {
	...
	public IMatrix set(int column, int row, Number value) {
		return this.set(column, row, value.doubleValue());
	}
	...
	@Override
	public Matrixd copyData(int[][] data) {
	...
	@Override
	public Matrixd copyData(long[][] data) {
		if( !unsafe && (data.length != this.getNumberColumns() || data[0].length != this.getNumberRows()) ) {
			throw new IllegalArgumentException("Input array length invalid: " + data.length + "," + data[0].length + ". Should be " + getNumberColumns() + "," + getNumberRows());
		}
		for(int i=0; i<this.getNumberColumns(); i++) {
	...
	@Override
	public Matrixd copyData(float[][] data) {
		if( !unsafe && (data.length != this.getNumberColumns() || data[0].length != this.getNumberRows()) ) {
			throw new IllegalArgumentException("Input array length invalid: " + data.length + "," + data[0].length + ". Should be " + getNumberColumns() + "," + getNumberRows());
		}
		for(int i=0; i<this.getNumberColumns(); i++) {
	...
	@Override
	public Matrixd copyData(double[][] data) {
		if( !unsafe && (data.length != this.getNumberColumns() || data[0].length != this.getNumberRows()) ) {
			throw new IllegalArgumentException("Input array length invalid: " + data.length + "," + data[0].length + ". Should be " + getNumberColumns() + "," + getNumberRows());
		}
		for(int i=0; i<this.getNumberColumns(); i++) {
	...
	@Override
	public Matrixd add(IMatrix mat) {
		...
		for(int j=0; j<this.getNumberRows(); j++) {
			for(int i=0; i<this.getNumberColumns(); i++) {
				this.data[i][j] += mat.getDouble(i, j);
			}
		}
		return this;
	}
	...
	@Override
	public Matrixd sub(IMatrix mat) {
		if(!unsafe && !compareSize(mat)) {
			throw new IllegalArgumentException("Input matrix size invalid: " + mat.getNumberColumns() + "," + mat.getNumberRows()
			+ ". Should be " + this.getNumberColumns() + "," + this.getNumberRows());
		}
	...
	@Override
	public Matrixd mul(IMatrix mat) {
		Matrixd.mul(this, mat, this, this.isUnsafe());
		return this;
	}

	
	@Override
	public Matrixd mul(IMatrix mat, IMatrix dest) {
		Matrixd.mul(this, mat, dest, this.isUnsafe());
		return this;
	}
	...
	public IMatrix mulLeft(IMatrix mat) {
		Matrixd.mul(mat, this, this, this.isUnsafe());
		return this;
	}
	...
	public IMatrix mulLeft(IMatrix mat, IMatrix dest) {
		Matrixd.mul(mat, this, dest, this.isUnsafe());
		return this;
	}
	...
	public boolean compare(IMatrix mat) {
		...
		for(int j=0; j<this.getNumberRows(); j++) {
			for(int i=0; i<this.getNumberColumns(); i++) {
				if( !MathUtils.isNearlyEqual(this.data[i][j], mat.getDouble(i, j), true) ) {
					return false;
				}
			}
		}
	...
	public Matrixd transpose() {
		double[][] dataTransp = new double[getNumberColumns()][getNumberRows()];
		for(int i=0; i<getNumberColumns(); i++) {
			for(int j=0; j<getNumberRows(); j++) {
	...
	public boolean isUpperTriangular() {
		...
		for(int row=0; row<getNumberRows(); row++) {
			for(int col=0; col<getNumberColumns(); col++) {
				if(row > col && !MathUtils.isNearlyEqual(this.data[col][row], 0.0)) {
					return false;
				}
			}
		}
	...
	@Override
	public Matrixd toUpperTriangle() {
		if(!unsafe && !isSquare()) {
			throw new IllegalArgumentException("Matrix must be square.");
		}
	...
	private static double determinant(double[][] matrix, boolean unsafe) {
		...
		for(int i=0; i<n; i++) {
			double[][] smaller = new double[n-1][n-1];
	...
	public String toString() {
		...
		sb.append("Matrix (double), " + this.getNumberColumns() + "x" + this.getNumberRows()).append(" (").append(Integer.toHexString(this.hashCode())).append(")");
	...
	@Override
	public Matrixd copy() {
		return new Matrixd(getNumberColumns(), getNumberRows()).copyData(this.data);
	}
	...
	@Override
	public Matrixd setUnsafe(boolean unsafe) {
		this.unsafe = unsafe;
		return this;
	}
